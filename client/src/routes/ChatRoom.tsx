...
export const socket = io("https://group-chat-app-poq9.onrender.com");
...
function ChatRoom() {
    ...
    useEffect(() => {
        if(room.roomId) {
            axios.get(`https://group-chat-app-poq9.onrender.com/room/${room.roomId}/chats`)
            .then((res) => {
                console.log(res);
    ...
    const exitRoom = async() => {
        ...
        if(room.roomId && user.email) {
            try {
                const response = await axios.post("https://group-chat-app-poq9.onrender.com/room/exit", {
                    "userEmail":  user.email,
                    "roomId": room.roomId
                });
